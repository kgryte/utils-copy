...
function deepCopy( val, copy, cache, refs, level ) {
	...
	if ( name === 'Set' || name === 'Map' ) {
		console.warn( 'not supported. Copying Sets and Maps is not currently supported. File an issue, submit a pull request, or contact the author directly at kgryte@gmail.com.' );
		return {};
	}
	...
	if ( level > 0 ) {
		...
		if ( name === 'Array' ) {
			...
			for ( j = 0; j < keys.length; j++ ) {
				key = keys[ j ];
				desc = Object.getOwnPropertyDescriptor( val[ key ], key );
